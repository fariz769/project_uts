...
title = tk.Label(frame, text="TCP Client", font=("Helvetica", 16, "bold"), bg="#f0f0f0", fg="#333")
title.grid(row=0, column=0, columnspan=3, pady=(0, 10))

status_label = tk.Label(frame, text="Status: Tidak Terhubung", font=("Helvetica", 10), bg="#f0f0f0", fg="red")
...
def connect_to_server():
    try:
        ...
        status_label.config(text="Status: Terhubung ke Server", fg="green")
        threading.Thread(target=receive, daemon=True).start()
    except:
        status_label.config(text="Status: Gagal Terhubung", fg="red")
